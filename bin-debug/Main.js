...
var Main = (function (_super) {
    ...
    Main.prototype.createGameScene = function () {
        var backgroundMask = new egret.Shape();
    ...
    Main.prototype.launchAnimations = function () {
        ...
        this.stage.addEventListener(egret.TouchEvent.TOUCH_TAP, function () {
            _this._nAnimMode = (_this._nAnimMode + 1) % 3;
        }, this);
        this._nScaleBase = 0;
        this.addEventListener(egret.Event.ENTER_FRAME, function (evt) {
            switch (_this._nAnimMode) {
                case AnimMode.ANIM_ROT:
                    _this._cartoon.rotation += Main.STEP_ROT;
                    break;
                case AnimMode.ANIM_SCALE://缩放范围在 0.5-1 之间
                    _this._cartoon.scaleX = _this._cartoon.scaleY = 0.5 + 0.5 * Math.abs(Math.sin(_this._nScaleBase += Main.STEP_SCALE));
                    break;
            }
            _this._txInfo.text =
                "选择角度：" + _this._cartoon.rotation
                    + "\n缩放比例：" + _this._cartoon.scaleX.toFixed(2)
                    + "\n\n轻触进入" + (["旋转", "缩放", "静止"][_this._nAnimMode]) + "模式";
            return false;
        }, this);
    };
    // 旋转设定
